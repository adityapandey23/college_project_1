# pc_step3_stats_and_fusion.py
# Pull MJPEG from Pi, run CNN + statistical model (FAN-based landmarks), fuse, overlay.
import time, math
...
def build_cnn_from_ckpt(ckpt_path, device):
    ...
    arch = ckpt.get("arch", "resnet18"); img_size = ckpt.get("img_size", 224)
    class_names = ckpt.get("class_names", CLASSES)
    m = {"resnet18":models.resnet18,"resnet34":models.resnet34,"resnet50":models.resnet50}[arch](weights=None)
...
def center_crop_resize(pil_img, out_size):
    w,h = pil_img.size; s=min(w,h); L,T=(w-s)//2,(h-s)//2
...
def predict_cnn_from_bgr(bgr, model, img_size, class_names, device):
    ...
    x = T.Compose([T.ToTensor(), T.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])])(pil).unsqueeze(0).to(device)
...
# ---------- Stats (face-alignment FAN) ----------
# Setup landmark detector once
...
def fuse_probs(cnn_probs: dict, stats_probs: dict, classes: list, alpha=None, min_floor=0.05):
    def norm(d): s = sum(d.values()) + 1e-8; return {k: max(0.0, v) / s for k, v in d.items()}
    c = norm(cnn_probs); s = norm(stats_probs)
    if alpha is None:
        wc, ws = max(c.values()), max(s.values()); tot = wc + ws + 1e-8
...
def main():
    ...
    if not cap.isOpened(): raise RuntimeError(f"Cannot open stream: {URL}")
